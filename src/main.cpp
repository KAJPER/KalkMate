...
void setup() {
    ...
    // Klawiatura matrycowa (MCP23017) — przed UI
    if (!inputBegin()) {
        Serial.println("[FATAL] MCP23017 nie odpowiada na 0x20!");
        Serial.println("        Klawiatura nie bedzie dzialac. Kontynuuje, ale UI bez nawigacji.");
    } else {
        Serial.println("[OK] Klawiatura matrycowa OK");
    }

    // OLED
    u8g2.begin();
    showSplashScreen(u8g2);
    Serial.println("[OK] OLED start");

    // Auto-reconnect WiFi w tle
    {
        char ssid[33] = "";
        char pass[64] = "";
